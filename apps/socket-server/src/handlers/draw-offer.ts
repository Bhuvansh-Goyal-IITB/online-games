...
export const drawOfferHandler: (server: Server) => EventHandler = (server) => {
  return async (ws, data) => {
    ...
    try {
      ...
      if (gameObject.getPlayerColor(id) == "w") {
        server.sendMessageTo(gameObject.getPlayerId("b")!, "draw offer");
      } else {
        server.sendMessageTo(gameObject.getPlayerId("w")!, "draw offer");
      }
    } catch (error) {
      server.sendMessage(ws, "error", {
